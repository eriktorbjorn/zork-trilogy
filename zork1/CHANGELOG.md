...
* `OTVAL-FROB` is the method used to calculate the value of the treasures in the trophy case. It traverses the contents recursively. However, it only counts the value of the top level objects. That's different from how Mainframe Zork worked, so I have fixed it. That means that e.g. putting the unopened egg in the trophy case now gives you points for both the egg and the canary inside. But you still need to open the egg to get the bauble, so this is not exploitable.
