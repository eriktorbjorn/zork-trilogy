...
<DEFMAC PROB ('BASE? "OPTIONAL" 'LOSER?)
	<COND (<ASSIGNED? LOSER?> <FORM ZPROB .BASE? .LOSER?>)
	      (ELSE <FORM G? .BASE? '<RANDOM 100>>)>>

<ROUTINE ZPROB
	 (BASE LOSER)
	 <COND (,LUCKY <G? .BASE <RANDOM 100>>)
	       (<NOT <EQUAL? .LOSER T>> <G? .LOSER <RANDOM 100>>)
...
<DEFAULT-DEFINITION INHIBIT-AUTO-TAKE?
	<DEFMAC INHIBIT-AUTO-TAKE? ('OBJ)
		<FORM FSET? .OBJ ',TRYTAKEBIT>>>
